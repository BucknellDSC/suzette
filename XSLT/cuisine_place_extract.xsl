...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:text>"Chapter", "Section", "Ref", "SameAs", "Key", "Place"</xsl:text>
    ...
    <xsl:function name="jc:createCSV" as="xs:string">
        ...
        <xsl:variable name="ref">
            <xsl:value-of select="jc:csvEscapeDoubleQuotes($node/@ref)" />
        </xsl:variable>
        <xsl:variable name="sameAs">
            <xsl:value-of select="jc:csvEscapeDoubleQuotes($node/@sameAs)"/>
        </xsl:variable>
        ...
        <xsl:variable name="output">
            ...
            <xsl:value-of select="jc:csvEscapeDoubleQuotes($chapter-n), jc:csvEscapeDoubleQuotes($divType), $ref, $sameAs, $key, $elementContent" separator="{$sep}"/>
            <xsl:value-of select="$terminal"/>
        </xsl:variable>
