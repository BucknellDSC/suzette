...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  -->
    ...
    <xsl:template match="/">
            ...
            <xsl:text>"Object", "Type", "Subtype", "Key", "elementContent"</xsl:text>
            <xsl:for-each select="//text//objectName">
    ...
    <xsl:function name="jc:createCSV" as="xs:string" >
      ...
      For those which might have multiple instances, provide a separator; -->
        ...
        <xsl:variable name="subtypeAttributeIfPresent">
            <xsl:value-of select="$node/@subtype/jc:csvEscapeDoubleQuotes(.)"/>
        </xsl:variable>
        <xsl:variable name="keytypeAttributeIfPresent">
            <xsl:value-of select="$node/@key/jc:csvEscapeDoubleQuotes(.)"/>
        </xsl:variable>
        ...
        <xsl:variable name="output">
            ...
            <xsl:value-of select="$elementName, $typeAttributeIfPresent, $subtypeAttributeIfPresent, $elementContent" separator="{$sep}"/>
            <xsl:value-of select="$terminal"/></xsl:variable>    
        ...
        <xsl:variable name="output">
            <xsl:value-of select="$terminal"/>
            <xsl:value-of select="$tag, $type, $subtype, $key, $elementContent" separator="{$sep}"/>
            <xsl:value-of select="$terminal"/>
        </xsl:variable>    
